...
export default function ArtikelList() {
  ...
  return (
    <div className="py-8 md:py-12 px-4 max-w-6xl mx-auto space-y-10">
          ...
          <button
            ...
            className={`px-4 py-2 text-xs md:text-sm font-semibold rounded-full border transition-standard focus-ring ${
              selectedCategory === cat
                ...
                : 'bg-card border-primary/10 text-muted-foreground hover:border-secondary hover:text-secondary'
            }`}
          >
            {cat}
          </button>
      ...
      {featuredArticle && (
        <div className="bg-card border border-primary/5 hover:border-secondary/20 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-standard card-interactive relative text-left flex flex-col lg:flex-row gap-8 p-6 md:p-8">
          ...
          <div className="flex flex-col justify-between py-2 flex-grow space-y-4">
            <div className="space-y-3">
              ...
              <h2 className="font-display text-2xl md:text-3xl text-primary font-bold hover:text-secondary transition-standard leading-tight">
                <button
                  onClick={() => navigate('artikel-detail', featuredArticle.slug)}
        ...
        <div className="space-y-6 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {regularArticles.map((post) => (
              <article
                key={post.slug}
                className="bg-card border border-primary/5 hover:border-secondary/35 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-standard flex flex-col justify-between card-interactive"
              >
